...
This plugin adds a new validates_as_eu_vat_number class method toActiveRecord::Base

The validator checks the attribute(s) given to see if they are valid EU VAT numbers. The :with modifier is either a field containing a country code, or the country code itself.
...
= Examples
  class VatNumber < ActiveRecord::Base
    ...
    validates_as_eu_vat_number :full_vat_number, :unless => Proc.new {|user| user.full_vat_number.blank? }
